...
set statusline=\ %{HasPaste()}%F%m%r%h\ [%{strlen(&ft)?&ft:'none'}]%=Line:\ %l/%L\ Column:\ %c

let &titlestring = "vim - " . $PWD . "/" . expand("%:t")
if &term == "screen"
        set t_ts=^[k
        set t_fs=^[\
endif
...
" comment header function
function CH(string)
  let comment = "// ---- [ " + a:string + " ] "
...
autocmd BufNewFile *.{js} call <SID>insert_gates()
